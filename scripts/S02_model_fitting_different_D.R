# S02_model_fitting_different_D.R
# Supplementary: fits models using an exponential distance kernel (alternative D
# formulation). Confirms model ranking is robust to choice of distance function.
#
# Input:  data/derived/model_input.csv
#         (from 02_prepare_model_input.ipynb)
# Output: outputs/model_fitting/model_comparisons_different_D.csv

library(here)

# ModelComparisons-TestModelTypes-ExpD.R
# Exponential distance kernel with fitted dc (optimizes log_dc)
# Copy-paste into a separate file so your original code stays untouched.
...
# ================================
# PER-FRAME BUILDER
#  Distance kernel: exp(-d/dc) where dc is fitted (passed in).
#  Poolings: sum, mean, max, nearest (NO 'rescale').
# ================================
calculate_decision_and_response <- function(df, s_times, tau = 5, b0, b1, c, dc,
                                            detection_delay = 5,
                                            include_speed = 1,
                                            include_density = 1,
                                            pooling = "sum") {
  ...
  for (i in seq_len(nrow(unique_fish))) {
    ...
    neighbour_proximity   <- input_data$Focal_Neighbour_Proximity[1]
    ...
    for (t in seq_len(nT)) {
      ...
      if (length(valid) > 0) {
        ...
        # distance kernel: exponential with fitted dc (meters)
        dc_eff <- max(dc, 1e-9)
        dist_fac <- exp(-d_m / dc_eff)
    ...
    local_densities[[i]] <- rep(neighbour_proximity, nT)
...
# ================================
# OBJECTIVE (HAZARD ONLY; no rescale)
#  Parameter order:
#   1) log_dc (always)
#   2) tau (if include_time)
#   3) b0  (if include_density)
#   4) b1  (always)
#   5) c   (if include_speed)
# ================================
objective_function <- function(params, df, s_times, settings) {
  idx <- 1

  log_dc <- params[idx]; idx <- idx + 1
  dc <- exp(log_dc)
  ...
  res <- calculate_decision_and_response(
    df, s_times, tau = tau, b0 = b0, b1 = b1, c = c, dc = dc,
  ...
  y  <- res$response
  ld <- res$neighbour_proximity

  D_in <- pmax(res$D_i_t, 0)
  lambda <- 1e-5 + b1 * D_in
  if (settings$include_density == 1) lambda <- lambda * exp(b0 * ld)
...
name_model <- function(row){
  ...
  paste0(row$model_core, "_", suffix, "_expD")
}
...
for (model_name in names(model_variants)) {
  ...
  init <- numeric(0)
  init <- c(init, log_dc = log(0.10))      # start: 10 cm
  if (settings$include_time == 1)  init <- c(init, tau = 12.0)
  if (settings$include_density == 1) init <- c(init, b0 = -5.0e-3)
  ...
  fit <- tryCatch({
    ...
    # log_dc bounds (meters): [0.005 m, 5 m]
    lower <- c(lower, log_dc = log(0.005))
    upper <- c(upper, log_dc = log(5.0))
    ...
    # parameter count k (for AIC)
    k <- 1 +                                    # log_dc
         (settings$include_time == 1) +
    ...
    data.frame(
      ...
      pooling  = settings$pooling,
      log_dc   = getp("log_dc"),
      dc_m     = if ("log_dc" %in% names(pars)) exp(getp("log_dc")) else NA_real_,
      tau      = getp("tau"),
      b0       = getp("b0"),
      b1       = getp("b1"),
      c        = getp("c"),
    ...
    data.frame(
      ...
      log_dc = NA, dc_m = NA, tau = NA, b0 = NA, b1 = NA, c = NA,
...
# SAVE
# ================================
outfile <- file.path(OUT_PATH, "model_comparisons_different_D.csv")
